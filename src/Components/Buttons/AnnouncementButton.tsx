...
export default function AnnouncementButton({
  ...
  const { likeCount, dislikeCount, userChoice, handleLike, handleDislike } =
    useEngagementButton({
      ...
      addDislikedMutation:
        api.announcement.addDislikeAnnouncement.useMutation(),
    });
    ...
    <>
      <div className="flex-end isolate inline-flex rounded-md shadow-sm">
        <button
          ...
          className={`focus group relative inline-flex items-center rounded-l-md px-2 py-2 text-base focus:z-10 
                ${
                  userChoice.like
